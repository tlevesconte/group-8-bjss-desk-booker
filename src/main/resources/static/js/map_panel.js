...
function main(){
	...
	//Send request to server, let server decide what to do
		if(type){//room
			...
			if(typeof table_changes[0][table_pos[0][index][4]] !== 'undefined'){ //If there are changes about this desk before
				table_changes[0][table_pos[0][index][4]][Object.keys(array_item)[0]] = array_item[Object.keys(array_item)[0]]; //If the date isn't written
				return;
			}
			table_changes[0][table_pos[0][index][4]]= array_item;
		}
	}
		...
		if(revert_item[0][0] >= 3){//if the change is about desk, process separately 
			switch (revert_item[0][2]){//mode
				...
				case 1://delete
					...
					var deleted_index = table_pos[1][revert_item[0][0]-3].indexOf(revert_item[1][0][4]);
					console.log(deleted_index,revert_item[1][0][4]);
					if(deleted_index != -1){
						table_pos[1][revert_item[0][0]-3].splice(deleted_index,1);//Remove the index from deleted index array
						var select_table= document.getElementById("support").lastChild;
						select_table.remove(deleted_index);
					}
					break;			
				case 2://Edit
	...
	function create_gadget(){ //Gadget for admin to save/cancel/revert/change mode
		...
		sub_button.src = "./js/icons/submit-icon.png";
		sub_button.width = 50;
		sub_button.addEventListener("click", sub_to_ser);
		function sub_to_ser(){
			// create_str_array('/insertDeskpos');
			send_desk_pos();
			send_canvas_item();
			location.reload();
			function send_desk_pos(){
				...
				// table_pos.splice(0,0,cities);
				// table_pos[2].splice(0,0,-1);
				var output =[cities,table_pos];
				output[1][1].splice(0,0,-1);
				xhttp.send("Deskpos=" + output);
			}
			function send_canvas_item(){
				var xhttp = new XMLHttpRequest();
				var url = '/insertCanvas';
				xhttp.open("POST", url, true);
				xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				xhttp.onreadystatechange = function(){
					if (xhttp.readyState === 4 && xhttp.status === 200) {
				...
				var no_icon = canvas_item[2].length;
				var no_text = canvas_item[1].length;
				var no_line = canvas_item[0].length;
				var output = [[cities,no_line,no_text,no_icon],canvas_item];
				xhttp.send("canvas_item=" + output);
			}
		}
		// function create_str_array(url){
	...
	function desk_adding(){ //Add desk if admin click for new desk
		...
		if(select_index == table_pos[1][0].length-1){
			table_pos[1][0].push(parseInt(select_table.value)+1); //Append the next index
			var new_option = document.createElement("option");
			new_option.text = parseInt(select_table.value)+1
			select_table.add(new_option);
		}
		table_pos[1][0].splice(select_index,1);
		select_table.remove(select_index);
		select_table.selectedIndex = table_pos[1][0].length-1; //Select the last index
	...
	function desk_delete(){ //Delete a desk if admin click delete key
		...
		if(event.which == 82){
			if(table_pos[0][index][3] <10){
				table_pos[0][index][3] +=20; //Normal to restricted case
			}
			table_pos[0][index][3] -=10; //Restricted to normal case
	...
	function check_desk_arr_conse(type){ //Combine the consecutive index at the end of the array
		var cache = table_pos[1][0].at(-1);
		var select_table = document.getElementById("support").lastChild;
		for(var i = table_pos[1][0].length-2; i >-1 ; i--){
			console.log(table_pos[1][0][i]);
			if(cache != table_pos[1][0][i] +1){ //Break if not consecutive
				return;
			}
			console.log("deleted index", cache)
			select_table.remove(table_pos[1][0].length-1);
			cache = table_pos[1][0][i];
			table_pos[1][0].pop();
	...
	function desk_array_delete(type, pos){//type: normal desk/ meeting room, pos: position that want to delete
		...
		for(var i = table_pos[1][0].length-1; i >-1 ; i--){
			if(table_pos[1][0][i] < table_pos[type][pos][4]){
				append_index = i+1; 
				break;
			}
		}
		...
		console.log(append_index,table_pos[1][0].at(-1)-1)
		// if(append_index == table_pos[1][0].at(-1)-1){//If it is the last item in the array
		// 	console.log("fucked")
		// 	select_table.remove(append_index);
		// }
		table_pos[1][0].splice(append_index, 0, table_pos[type][pos][4]);//Add the index in sorted array
	...
	function desk_selection_admin(){ //Allow admin reserve desk/ modify booked desk
		...
		var row = [];
		var table_item = [];
		container.childNodes[index].lastChild.src = "./js/icons/3"+ table_pos[0][index][3]%10+".png"; //Change to purple icon
		if(table_pos[0][index][3] >= 10){
			row[0] = document.createElement("tr");
			row[0].innerHTML = "This desk is locked by admin, unlock by clicking D key"
		...
		for(var i = 1; i < 8; i++){ //7 days
			...
			reserve_button.innerHTML = `<font size = "1">Reserve</font>`;
			cancel_button.innerHTML = `<font size = "1">Cancel</font>`;
		...
		var tem_save = document.createElement("button");
		tem_save.addEventListener("click", overwrite_booking);
		tem_save.innerHTML= "Update booking in server";
		function overwrite_booking(){
			console.log(table_changes);
			send_desk_pos();
			function send_desk_pos(){
				var xhttp = new XMLHttpRequest();
				var url = '/AdminOverwrite';
				xhttp.open("POST", url, true);
				xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				xhttp.onreadystatechange = function(){
					if (xhttp.readyState === 4 && xhttp.status === 200) {
				...
				var output = [];
				output[0] = cities;
				for(var i = 0; i < Object.keys(table_changes[0]).length; i++){
					for(var j = 0; j < Object.keys(table_changes[0][Object.keys(table_changes[0])[i]]).length; j ++){
						output.push(Object.keys(table_changes[0])[i]); //Index of desk
						output.push(Object.keys(table_changes[0][Object.keys(table_changes[0])[i]])[j]); //Date
						output.push(table_changes[0][Object.keys(table_changes[0])[i]][Object.keys(table_changes[0][Object.keys(table_changes[0])[i]])[j]]); //Operation

					}
				}
				console.log(output)
				// table_changes.splice(0,0,cities);
				xhttp.send("TableChanges=" + (output));
				table_changes = [{}];
			}
		}
		table.appendChild(tem_save);
	...
	function desk_tool(){
		...
		for (var i =0; i < table_pos[1][0].length; i++){
			var index_option = document.createElement("option");
			console.log(table_pos[1][0][i]);
			index_option.text = table_pos[1][0][i];
	...
	function create_toolbar(){
		...
		for(var i=0 ; i<4; i++){
			...
			tool[i].type = "image";
			tool[i].src = "./js/icons/ma"+(i+1)+".png";
			tool[i].width = 75;
			tool[i].height = 75;
